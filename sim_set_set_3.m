function distance = sim_set_set_3(pair, path, load_names, meanFeature, projection, inter_s, intra_s, type)
...
%% only update the mean and variance for joint and single
...
for i = 1 : num
    ...
    intra_s_joint = (epson - repmat(mean(epson,1), totalNum,1))'*(epson - repmat(mean(epson,1), totalNum,1))/totalNum;
    ...
    % update f1
    [tmpF, tmpG] = inv_covariance(inter_s, intra_s, numSelect);
    tmp_sum = sum(f1);
    meanF1 = (tmp_sum*tmpF' + numSelect*tmp_sum*tmpG')*inter_s';
    epson1= (f1*tmpF' + repmat(tmp_sum, numSelect, 1)*tmpG')*intra_s';
    intra_s_update1 = (epson1 - repmat(mean(epson1,1), numSelect,1))'*(epson1 - repmat(mean(epson1,1), numSelect,1))/numSelect;
    
    
    ind = 1;
    if numSelect > numFaces || ind
        [F2, G2] = inv_covariance(inter_s, intra_s_joint, numSelect);
    ...
    if numSelect > numFaces || ind
        [F1, G1] = inv_covariance(inter_s, intra_s_update1, numSelect);
    ...
    [tF2, tG2] = inv_covariance(t_inter_s, intra_s_joint, numSelect);
    
    tmp1 = trace(center_f1*tF2*center_f1') + sum(center_f1)*tG2*sum(center_f1)';
    
    tmpCenter_f1 = f1 - repmat(meanF1, numSelect, 1);
    tmp2 = trace(tmpCenter_f1 *F1*tmpCenter_f1') + sum(tmpCenter_f1)*G1*sum(tmpCenter_f1)';
