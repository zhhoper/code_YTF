...
%         [id, video, noise] = get_var_video_EM_corr(path, faceLabel, videoLabel, load_names, type, meanFeature, projection);
    ...
    %[ori_intraPre.withoutCross, ori_extraPre.withoutCross] = get_precision(distanceOri.withoutCross, distanceOri.label);
    %[ori_intraPre.withoutCross, ori_extraPre.withoutCross] = get_precision(distanceOri.withoutCross, distanceOri.label);
    [ori_intraPre.combine, ori_extraPre.combine] = get_precision(distanceOri.combine, distanceOri.label);
    [ori_intraPre.combine_adapt, ori_extraPre.combine_adapt] = get_precision(distanceOri.combine_adapt, distanceOri.label);
%     fprintf('Done!\n');
...
%         ori_intraPre.selectAdapt, ori_extraPre.selectAdapt);
    best_ori = drawROC_new(ori_intraPre, ori_extraPre);
    fprintf('Done!\n');
    cccc = 0;
