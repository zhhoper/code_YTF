function [id, video, noise] = get_var_video_decorrelate(path, faceLabel, videoLabel, load_names, type, meanFeature, projection, numFrames)
...
for i = 1 : numFaces
    ...
    for j = 1 : tmpNum
        ...
        % number of frames for each video
        tmpNum1 = size(tmpFeature,1);
        numUseFrames = numUseFrames + tmpNum1;
        
        tmpFeature = (tmpFeature - repmat(meanFeature, tmpNum1, 1))*projection; 
        tmpMean = mean(tmpFeature, 1);
        varVideo = varVideo + (tmpMean - meanEachFace(i,:))'*(tmpMean - meanEachFace(i,:));
        
        tmpf = tmpFeature - repmat(tmpMean, tmpNum1, 1);
        tmpf = decorre_feature(tmpf, tmpf, numFrames);
        varNoise = varNoise + tmpf'*tmpf;
    end
end
...
noise.var = varNoise/numUseFrames;
end