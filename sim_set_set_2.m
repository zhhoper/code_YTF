function distance = sim_set_set_2(pair, path, load_names, meanFeature, projection, inter_s, intra_s, type)
...
%% only update the mean and variance for joint
...
for i = 1 : num
    ...
    intra_s_update = (epson - repmat(mean(epson,1), totalNum,1))'*(epson - repmat(mean(epson,1), totalNum,1))/totalNum;
    ...
    if numSelect > numFaces || ind
        [F2, G2] = inv_covariance(inter_s, intra_s_update, numSelect);
    ...
    [tF2, tG2] = inv_covariance(t_inter_s, intra_s_update, numSelect);
